...
class PWLSpline(nn.Module):
    ...
    def _slopes_yk(self):
        ...
        avg = (slopes*seg_dx).sum(1, keepdim=True) / (seg_dx.sum(1, keepdim=True) + 1e-8)
...
class KVGeometryV(nn.Module):
    ...
    @torch.no_grad()
    def fit(self, calib_V: torch.Tensor, epochs: int = 8, lr: float = 2e-3, wd: float = 1e-4):
        x = calib_V
        ...
        loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(x_n), batch_size=bs, shuffle=True, drop_last=True)
        ...
        with torch.no_grad():
            ...
            self.Vk.copy_(Vk)
        self.geom = geom.eval()
        for p in self.geom.parameters(): p.requires_grad_(False)
        self.fitted = True
