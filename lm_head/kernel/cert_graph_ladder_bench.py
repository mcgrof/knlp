#!/usr/bin/env python3
"""Bench the adaptive static-ladder certified decode (eager correctness + graph)."""
...
from cert_graph_batch import CertDecodeGraphLadder
...
def main():
    ...
    codes, scale, _ = quantize_cols_codes(a_full, 8)
    codes = codes.to(dev); scale = scale.to(dev)
    aq_err_row = (a_full - codes.float() * scale.unsqueeze(0)).norm(dim=1).contiguous()
    aq_l2 = codes.float().pow(2).sum(1).sqrt().contiguous()
    WUt = W_U.t().contiguous()
    H = H.to(dev)
    N = H.shape[0]
    batches = [int(x) for x in args.batches.split(",")]
    print(f"[{args.model}] V={V} d={d} C={C} S={S} r={args.r} ladder-cum={sum([4,4,8,8,16,16,32,32,64,72])}", flush=True)
    ...
    for Bn in batches:
        ...
        g = CertDecodeGraphLadder(Bb, codes, scale, delta, aq_err_row, aq_l2, W_U, S, Bn, dev)
        false_cert = okf = tot = ncert = 0
        for p in pools:
            ids_b, cert_b = g.run_eager(p)
            gt = (p.float() @ WUt.float()).argmax(1)
            false_cert += int(((ids_b != gt) & cert_b).sum())
            okf += int((ids_b == gt).sum()); ncert += int(cert_b.sum()); tot += p.shape[0]
        ...
        try:
            ...
            print(f"  [graph FAILED B={Bn}] {type(ex).__name__}: {ex}", flush=True)
        row = {"batch": Bn, "dense_ms": dense, "graph_ms": graph_ms,
               "cert_rate": ncert / tot, "match": okf / tot, "false_cert": false_cert}
        if graph_ms:
            row["speedup_mean"] = dense["mean"] / graph_ms["mean"]
        res["rows"].append(row)
        gs = f"graph {graph_ms['mean']:.3f} ({dense['mean']/graph_ms['mean']:.2f}x)" if graph_ms else "graph FAIL"
        print(f"[B={Bn:>2}] dense {dense['mean']:.3f} {gs} cert {ncert/tot:.3f} "
              f"match {okf/tot:.3f} false_cert {false_cert}", flush=True)
