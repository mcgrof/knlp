...
if HAS_TRITON:
    ...
    def _trellis_evo_fwd_kernel(
        ...
        for c in range(nC):
            ...
            # --- LayerNorm(SiLU(z)) VJP w.r.t. z, carrying alpha (rowwise over M)
            sig = tl.sigmoid(z)
            s = z * sig
    ...
    def trellis_state_evolution_triton(write, alpha, P, rmat, gamma, chunk_size):
        """Triton fused forward. Returns M0s [B,H,nC,M,D], us [B,H,nC,C,M]."""
        M0s, us = _evo_fwd(write, alpha, P, rmat, gamma, chunk_size)
        return M0s, us
    ...
    def _evo_fwd(write, alpha, P, rmat, gamma, C):
        B, H, T, D = write.shape
        M = alpha.shape[-1]
        nC = P.shape[2]
        N = B * H
        ...
        return M0s.view(B, H, nC, M, D), us.view(B, H, nC, C, M)
    ...
    class TrellisStateEvolutionTriton(torch.autograd.Function):
        ...
        @staticmethod
        def forward(ctx, write, alpha, P, rmat, gamma, chunk_size):
            B, H, T, D = write.shape
            M = alpha.shape[-1]
            C = chunk_size
            nC = P.shape[2]
        ...
        def backward(ctx, grad_M0s, grad_us):
            ...
            grad_alpha = gA.view(B, H, nC * C, M)[:, :, :T].contiguous()
            ...
            return grad_write, grad_alpha, grad_P, grad_rmat, grad_gamma, None
