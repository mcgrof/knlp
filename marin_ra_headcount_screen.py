...
ROOT = Path('/data/results-archive/marin-ra-headscale') / f'7b-{TIMESTAMP}'
...
def reciprocal_mix_cache(past, selected_layers, beta=0.2, mode='mixed'):
    ...
    if hasattr(past, 'layers'):
        ...
        for layer_idx, (k_src, v_src) in enumerate(zip(past.key_cache, past.value_cache)):
            k = k_src.clone()
            v = v_src.clone()
            if layer_idx in selected_layers:
                kf = F.normalize(k.float(), dim=-1)
...
def eval_variant(model, tokens, selected_layers, beta, mode, label,
                 prompt_len=2048, wall_cap=WALL_CAP_SECONDS):
    ...
    while True:
        ...
        input_ids = torch.tensor(
            tokens[start_idx:start_idx + prompt_len],
            dtype=torch.long
        ).unsqueeze(0).to(DEVICE)
        next_tok = torch.tensor(
            [[tokens[start_idx + prompt_len]]],
            dtype=torch.long
        ).to(DEVICE)
        ...
        with torch.no_grad():
            ...
            if selected_layers is not None and len(selected_layers) > 0:
                transformed_cache = reciprocal_mix_cache(
                    out_fp.past_key_values, selected_layers,
                    beta=beta, mode=mode
                )
            else:
                transformed_cache = reciprocal_mix_cache(
        ...
        # Logit error
        err = (o_base.logits[0, 0].float() - o_var.logits[0, 0].float()).abs().max().item()
        agree = 1 if o_base.logits[0, -1].argmax() == o_var.logits[0, -1].argmax() else 0
        ...
        if n_done % 5 == 0:
            log(f'  [{label}] {n_done} prompts done, elapsed {elapsed:.0f}s, '
    ...
    result = {
        ...
        'p95_logit_error': round(float(np.percentile(logit_errs, 95)), 6) if logit_errs else None,
...
def main():
    ...
    log(f'[model_resolved] {MODEL_NAME} -> {model_class} params, '
        f'{num_layers} layers, {num_heads} attn heads, hidden={hidden_size}')
    log(f'[note] Task says "7B-class" but actual model is {model_class}. '
    ...
    variants = [
        ...
        {'label': 'RA-20', 'n_heads': 20, 'layers': sorted(layer_ranking[:20]),
         'beta': 0.15, 'mode': 'mixed'},
        {'label': 'RA-24', 'n_heads': 24, 'layers': sorted(layer_ranking[:24]),
         'beta': 0.15, 'mode': 'mixed'},
        {'label': 'RA-28', 'n_heads': 28, 'layers': sorted(layer_ranking[:28]),
         'beta': 0.15, 'mode': 'mixed'},
        {'label': 'RA-32', 'n_heads': 32, 'layers': sorted(layer_ranking[:32]),
         'beta': 0.15, 'mode': 'mixed'},
    ]
    ...
    config = {
        ...
        'variants': [
            {'label': v['label'], 'n_heads': v['n_heads'],
             'layers': v['layers'], 'beta': v['beta'], 'mode': v['mode']}
            for v in variants
        ],
        'timestamp': TIMESTAMP,
    }
    ...
    for vi, var in enumerate(variants):
        log(f'[variant_start] {var["label"]} ({vi+1}/{len(variants)}) '
        ...
        log(f'[variant_finish] {var["label"]} '
        ...
        log(f'[partial_save] Wrote partial results ({len(all_results)}/{len(variants)} done)')
    ...
    for r in all_results:
        ...
        delta_pct = (delta / baseline_ppl * 100) if delta is not None and baseline_ppl else None
    ...
    # Print summary table
    log('[summary_table]')
    log(f'  {"Variant":<12} {"PPL":>10} {"delta":>10} {"delta%":>10} {"agree":>10} {"prompts":>8} {"wall_s":>8}')
    for s in summary_lines:
        log(f'  {s["label"]:<12} {s["avg_ppl"]:>10} '
