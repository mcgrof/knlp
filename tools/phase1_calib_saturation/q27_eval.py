...
"""Qwen3.6-27B sym vs asym FP8 KV cache evaluation.

Note: Qwen3.6-27B is a hybrid linear+full-attention multimodal model
(arch=Qwen3_5ForConditionalGeneration, layer_types alternate
linear_attention/full_attention with full every 4th).  KV-cache
quantization here only meaningfully affects the full-attention layers
(linear attention layers don't carry the same KV cache tensor).  Running
the same WikiText-2 PPL + GSM8K protocol used on Qwen2.5-7B in
qfb_round1.py.

One config per Python process.
"""
import argparse
import json
import math
...
def gsm8k_eval(llm, n=200, n_shot=8):
    ...
    shots = "\n\n".join(
        f"Question: {ex['question']}\nAnswer: {ex['answer']}" for ex in train)
...
def main():
    ...
    try:
        ...
        print(f"  PPL ppl={ppl_res['ppl']:.4f} n_toks={ppl_res['n_tokens']} "
              f"t={ppl_res['eval_time_s']:.1f}s", flush=True)
    except Exception as e:
    ...
    record = dict(
        ...
        ppl=ppl_res, gsm8k=gsm_res,
        T=args.ppl_T, gpu="H100",
    )
    with open(out_path, "a") as f:
