...
PATIENTS = os.environ.get(
    "PATIENTS", "patient_01 patient_02 patient_03 patient_05 patient_06").split()
...
def main():
    ...
    def make_cache(kv_list):
        nl = ac.n_layers
        ik = [torch.cat([kv_list[c][0][li] for c in range(len(kv_list))], dim=2)
              for li in range(nl)]
        iv = [torch.cat([kv_list[c][1][li] for c in range(len(kv_list))], dim=2)
              for li in range(nl)]
    ...
    assert all(c[0][0].shape[2] == kvt for c in carts), \
        "heterogeneous cart sizes: repeat_interleave masking assumes uniform kvt"
    _STATE["kvt"] = kvt
    print(f"[parity] N={N} carts, kv_tokens={kvt} (env KV_TOKENS={KV_TOKENS})",
          flush=True)
    ...
    for j, pid in enumerate(PATIENTS):
        ...
        onehot_alone = torch.zeros(1, device=DEVICE); onehot_alone[0] = 1.0
        onehot_all = torch.zeros(N, device=DEVICE); onehot_all[j] = 1.0
        onehot_perm = torch.zeros(N, device=DEVICE); onehot_perm[jp] = 1.0
        onehot_dup = torch.zeros(N, device=DEVICE); onehot_dup[0] = 1.0
        ...
        for q in patient.questions[:N_QUERIES]:
            ...
            ids = tok.apply_chat_template(
                ...
                return_tensors="pt", enable_thinking=True).to(DEVICE).flatten()
            ...
            if not (la.argmax() == lb.argmax() == lc.argmax()):
                argmax_mismatch += 1
            total += 1
            print(f"  [{pid} q{total}] max|A-B|={d_ab:.3f} max|A-C|={d_ac:.3f} "
    ...
    print(f"[parity] worst KL(A||B)={kl_ab:.5f} KL(A||C)={kl_ac:.5f} "
    ...
    ok = (argmax_mismatch == 0
          and worst_ab <= tol and worst_ac <= tol
          and kl_ab <= kl_tol and kl_ac <= kl_tol)
    print(f"POSITION_PARITY_{'PASS' if ok else 'FAIL'} "
