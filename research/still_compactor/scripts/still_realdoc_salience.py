...
from still_dense_controls import (
    still_cache,
    student_cont_logits,  # noqa
    student_letter_logits,
    eval_dense,
)
...
COMMON = set(
    "The This That These Those There Here When Where While What Which "
...
def cloze_questions(chunk, tok, rng, max_q):
    ...
    for sent in sents:
        if len(qs) >= max_q:
            break
        cands = [w for w in re.findall(r"\b[A-Z][a-z]{3,}\b", sent) if w in vocab]
...
def build_item(model, tok, ctx, qs, dev, topk=200, filter_q=True):
    ...
    kept = []
    for q in qs:
        qi = tok(q["q"], add_special_tokens=False, return_tensors="pt").input_ids.to(
            dev
        )
        if filter_q:  # keep only teacher-correct, no-context-wrong
            ...
            if not (tc and nw):
                continue
        coi = tok(
            q["cont"], add_special_tokens=False, return_tensors="pt"
        ).input_ids.to(dev)
...
def main():
    ...
    ap.add_argument("--cache-dir", default=os.path.expanduser("~/still_work/books"))
    ap.add_argument("--device", default="cuda:0")
    args = ap.parse_args()
    di = int(args.device.split(":")[1])
    ...
    print(
        f"real-doc salience: ctx={args.ctx_tokens} t={args.t_compact} "
    ...
    for k, t in texts.items():
        chunks += chunk_contexts(
            t, tok, args.ctx_tokens, (args.n_train + args.n_eval), rng
        )
    rng.shuffle(chunks)
    items = []
    for ch in chunks:
        ...
        if it and len(it["qs"]) >= 3:
            items.append(it)
    print(
        f"built {len(items)} usable contexts (teacher-filtered), "
        f"mean {sum(len(x['qs']) for x in items)/max(len(items),1):.1f} Q/ctx"
    )
    train, held = items[: args.n_train], items[args.n_train :]
    if len(held) < 4:
        print("not enough held-out; reduce n-train or max-q")
        return

    comp = STILLCompactorLayer(H, d, t=args.t_compact, base_theta=theta).to(
        dev, torch.bfloat16
    )
    dec = [p for p in comp.parameters() if p.dim() > 1]
    nod = [p for p in comp.parameters() if p.dim() <= 1]
    opt = torch.optim.AdamW(
        [{"params": dec, "weight_decay": 0.01}, {"params": nod, "weight_decay": 0.0}],
        lr=4e-5,
        betas=(0.9, 0.95),
    )
    ...
    for step in range(args.steps):
        ...
        loss = 0.0
        for q in qs:
            slg = student_cont_logits(
                model, cl, it["ctxlen"], q["q_ids"], q["cont_ids"], dev
            ).float()
            loss = loss + F.kl_div(
        ...
        if step % 200 == 0 or step == args.steps - 1:
            ...
            print(
                f"  step {step:>4} loss={loss.item():.3f} held-out acc={a:.3f} ({c}/{t})"
            )
    a, c, t = eval_dense(model, ev, held, dev)
    print(
        f"[real-doc FINAL] held-out salience acc = {a:.3f} ({c}/{t}) at "
