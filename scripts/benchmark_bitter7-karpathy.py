...
# Taking into account:
# @karpathy: he most dramatic optimization to nanoGPT so far (~25% speedup) is
# to simply increase vocab size from 50257 to 50304 (nearest multiple of 64).
# This calculates added useless dimensions but goes down a different kernel
# path with much higher occupancy. Careful with your Powers of 2.
...
# -------------------------------
# Bitter7-style pruner (optimized + padding)
# -------------------------------
...
class Bitter7PrunerOptimized:
    def __init__(
        ...
        pad_multiple: 0 = no padding, otherwise pad total length up to
                      nearest multiple of this value (e.g. 64, 128)
        """
        self.prunable_layers = prunable_layers
        ...
        # Precompute shapes
        self.total_params_real = sum(m.weight.numel() for _, m in self.prunable_layers)
        self.max_layer_elems = max(m.weight.numel() for _, m in self.prunable_layers)

        if self.pad_multiple and self.pad_multiple > 0:
            self.total_params_padded = (
                (self.total_params_real + self.pad_multiple - 1)
                // self.pad_multiple
            ) * self.pad_multiple
        else:
            self.total_params_padded = self.total_params_real
        ...
        self.all_importances = torch.empty(
            self.total_params_padded, device=self.device, dtype=self.imp_dtype
        )
    ...
    def _compute_importances(self) -> torch.Tensor:
        ...
        If padding is enabled, [total_params_real : total_params_padded]
        is filled with +inf so it won't affect bottom-k selection.
        """
        offset = 0
        for name, module in self.prunable_layers:
            w = module.weight.data
        ...
        # Pad tail with +inf so it never enters bottom-k
        if self.total_params_padded > self.total_params_real:
            self.all_importances[self.total_params_real :] = torch.inf
    ...
    def get_threshold(self) -> torch.Tensor:
        ...
        NOTE: k is computed from the REAL param count, not the padded one.
        """
        imps = self._compute_importances()
        numel_real = self.total_params_real
        k = int(self.sparsity * numel_real)

        if k <= 0:
            return torch.tensor(0.0, device=imps.device, dtype=torch.float32)
        if k >= numel_real:
            # Only consider real elements for max as well
            return imps[:numel_real].float().max()
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark magnitude pruning vs Bitter7-style pruning "
                    "with optimized memory usage and optional padding."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Pad importance length up to nearest multiple of this value "
             "(0 = no padding, e.g. 64 or 128 for nanoGPT-style trick).",
    )
    ...
    pad_label = (
        f"Bitter7 Optimized (scratch + {args.imp_dtype}"
        + (f", pad={args.pad_multiple})" if args.pad_multiple > 0 else ", no pad)")
    )
    benchmark_pruner(
