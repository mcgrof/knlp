...
import json, time, torch, sys, os, shutil, subprocess
...
if not HAS_CUDA:
    sys.exit("ERROR: tier0_baseline requires a CUDA GPU (torch.cuda unavailable)")
...
for model in MODELS:
    ...
    for seq_len in SEQ_LENS:
        ...
        out_path = os.path.join(
            OUTPUT_DIR, f"tier0_baseline_{model_short}_{seq_len}.json"
        )
...
# Save combined
with open(os.path.join(OUTPUT_DIR, "tier0_baseline_all.json"), "w") as f:
