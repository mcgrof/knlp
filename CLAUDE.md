...
### `/data/results-archive/` — Artifact Archive
...
```bash
cp -a results/v29/ /data/results-archive/bpa/results/v29/
cp bpa_v29_final_report.md /data/results-archive/bpa/
cd /data/results-archive && git add bpa/ && git commit
```
...
- After completion: move to `/data/results-archive/bpa-instructions/`
...
`/data/results-archive/bpaNNN/` with a consistent structure:

```
/data/results-archive/bpa48/
...
2. Archive results to results-archive
...
code and results-archive holds the full artifact history.
...
Cloud GPU spend (RunPod, Verda, etc.) is tracked in `results-archive` with
...
Create tags with the helper (run inside `results-archive`):
...
Key results archive: `prune:/data/results-archive/flashinfer-asym-e2e-20260427/`
